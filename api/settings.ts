...
async function handleUpdateSetting(req: VercelRequest, res: VercelResponse) {
  ...
  if (!contributionType || !['kas_kelas', 'amal_jumat', 'paguyuban_ngaji', 'lks', 'pisangisasi'].includes(contributionType)) {
    return sendError(res, 'Valid contribution type is required (kas_kelas, amal_jumat, paguyuban_ngaji, lks, pisangisasi)');
  }

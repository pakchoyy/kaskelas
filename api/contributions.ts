...
import { sendSuccess, sendError, sendNotFound, handleError, createId, parseQueryParam, parseQueryParamArray, parseQueryParamInt, isValidDate } from '../server/utils.js';
import type { Contribution, ContributionType } from '../server/types.js';
...
async function handleCreateContribution(req: VercelRequest, res: VercelResponse) {
  ...
  if (contributionType === 'tabungan_guru_bulanan') {
    ...
    if (typeof nominal !== 'number' || nominal <= 0) {
      return sendError(res, 'Tabungan Guru Bulanan nominal must be positive');
    }
  }
...
async function handleUpdateContribution(req: VercelRequest, res: VercelResponse) {
  ...
  if ((existing.contributionType === 'tabungan_guru_bulanan' || existing.contributionType === 'tabungan_guru_tw') && nominal !== undefined && (typeof nominal !== 'number' || nominal <= 0)) {
    return sendError(res, 'Tabungan Guru nominal must be positive');
  }
